...
1. Each fixture should call makeFixture to register the fixture and set its parent to 
   TestCase.
2. Each tests case should call makeTestCase to register the test case and set its parent 
   to the current fixture.
*)

on makeFixture(aUserFixture)
...
on makeTestCase()
	(* Create an unregistered text case that inherit from the curernt fixture. You can 
...
on makeTextTestRunner(aSuite)
	script TextTestRunner
		...
		property TestResult : makeTestResult(aSuite's name)
		...
		on run
			-- TestResult will update me while running the suite..
			TestResult's setObserver(me)
			TestResult's runTest(suite)
			printDefects("ERRORS", TestResult's errors)
			printDefects("FAILURES", TestResult's failures)
			printCounts()
			printResult()
		end run
		...
		on printTitle()
			printLine((TestResult's startDate) as string)
			printLine("")
			printLine(TestResult's name)
			printLine("")
		end printTitle
		...
		on printResult()
			printLine("")
			if TestResult's hasPassed() then
				printColoredLine("OK", successColor)
			else
